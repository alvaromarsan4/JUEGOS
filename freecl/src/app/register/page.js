    ...
    <div className="flex-grow flex items-center justify-center bg-background text-foreground relative overflow-hidden p-4 w-full h-full">
      ...
      <div className="w-full max-w-md bg-card backdrop-blur-md border border-border rounded-2xl shadow-2xl p-8 animate-fade-in-up">
        <div className="text-center mb-6">
          <h2 className="text-3xl font-bold mb-2 text-card-foreground">Crear <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500">Cuenta</span></h2>
          <p className="text-muted-foreground text-sm">Únete a la comunidad de Project Games</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          {message && <div className="p-3 bg-green-500/10 border border-green-500/50 rounded-lg text-green-600 text-sm text-center font-medium">{message}</div>}
          {errors && errors.general && <div className="p-3 bg-red-500/10 border border-red-500/50 rounded-lg text-red-600 text-sm text-center font-medium">{errors.general}</div>}
          ...
          <div className="space-y-1">
            ...
            <input type="email" placeholder="ejemplo@correo.com" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
              className={`w-full px-4 py-3 rounded-lg bg-background border ${errors?.email ? 'border-red-500' : 'border-input'} text-foreground placeholder-muted-foreground/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all shadow-sm`} />
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-3 pr-10 rounded-lg bg-background border ${errors?.password ? 'border-red-500' : 'border-input'} text-foreground placeholder-muted-foreground/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all shadow-sm`}
              />
          ...
          <div className="flex justify-center pt-2">
            <ReCAPTCHA
              ...
              theme="light" // Importante: Tema light para que se vea bien en fondo claro (o system preference si soporta)
            />
          </div>

          <div className="pt-2">
            <button disabled={loading} className={`w-full py-3.5 px-4 bg-primary hover:bg-blue-700 text-primary-foreground font-bold rounded-lg shadow-lg shadow-blue-500/30 transform transition-all duration-200 hover:scale-[1.02] active:scale-95 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}>
        ...
        <div className="mt-6 text-center text-sm text-muted-foreground">
          <p>¿Ya tienes cuenta? <Link href="/login" className="text-primary hover:text-blue-600 font-semibold transition-colors">Inicia sesión aquí</Link></p>
        </div>
      </div>
    </div>
  );
